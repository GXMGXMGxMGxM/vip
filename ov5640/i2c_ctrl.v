...
(
    ...
    input   wire            addr_num    ,   //输入i2c寄存器地址字节数,拉高表示寄存器地址为16位
    input   wire    [15:0]  byte_addr   ,   //输入i2c寄存器地址
    input   wire    [7:0]   wr_data     ,   //输入i2c寄存器数据
...
// reg   define
reg     [7:0]   clk_cnt         ;   //系统时钟计数器,控制生成clk_i2c时钟信号
reg     [3:0]   state           ;   //状态机状态
reg             i2c_clk_cnt_en  ;   //cnt_i2c_clk计数器使能信号
reg     [1:0]   i2c_clk_cnt     ;   //clk_i2c时钟计数器,控制生成cnt_bit信号
reg     [2:0]   bit_cnt         ;   //sda比特计数器
reg             ack             ;   //应答信号
...
//***************************** Main Code ****************************//
//********************************************************************//
// clk_cnt:系统时钟计数器,控制生成clk_i2c时钟信号
always@(posedge sys_clk or negedge sys_rst_n)
    if(sys_rst_n == 1'b0)
        clk_cnt <=  8'd0;
    else    if(clk_cnt == CNT_CLK_MAX - 1'b1)
        clk_cnt <=  8'd0;
    else
...
// i2c_clk_cnt_en:cnt_i2c_clk计数器使能信号
always@(posedge i2c_clk or negedge sys_rst_n)
    if(sys_rst_n == 1'b0)
        i2c_clk_cnt_en  <=  1'b0;
    else    if((state == STOP) && (bit_cnt == 3'd3) &&(i2c_clk_cnt == 3))
        i2c_clk_cnt_en  <=  1'b0;
    else    if(i2c_start == 1'b1)
        i2c_clk_cnt_en  <=  1'b1;

// i2c_clk_cnt:i2c_clk时钟计数器,控制生成cnt_bit信号
always@(posedge i2c_clk or negedge sys_rst_n)
    if(sys_rst_n == 1'b0)
        i2c_clk_cnt <=  2'd0;
    else    if(i2c_clk_cnt_en == 1'b1)
...
always@(posedge i2c_clk or negedge sys_rst_n)
    if(sys_rst_n == 1'b0)
        ...
        bit_cnt <=  3'd0;
    else    if((bit_cnt == 3'd7) && (i2c_clk_cnt == 2'd3))
        bit_cnt <=  3'd0;
    else    if((i2c_clk_cnt == 2'd3) && (state != IDLE))
    ...
    if(sys_rst_n == 1'b0)
        ...
        START_1:
            if(i2c_clk_cnt == 3)
                ...
                state   <=  state;
        SEND_D_ADDR:
            if((bit_cnt == 3'd7) &&(i2c_clk_cnt == 3))
                state   <=  ACK_1;
            else
                state   <=  state;
        ACK_1:
            if((i2c_clk_cnt == 3) && (ack == 1'b0))
                begin
                    if(addr_num == 1'b1)
                        state   <=  SEND_B_ADDR_H;
                    else
...
always@(*)
    case    (state)
        ...
        ACK_3,WR_DATA,ACK_4,START_2,SEND_RD_ADDR,ACK_5,RD_DATA,N_ACK:
            if((i2c_clk_cnt == 2'd1) || (i2c_clk_cnt == 2'd2))
                i2c_scl <=  1'b1;
            else
                i2c_scl <=  1'b0;
        STOP:
            if((bit_cnt == 3'd0) &&(i2c_clk_cnt == 2'd0))
                i2c_scl <=  1'b0;
            else
                i2c_scl <=  1'b1;
...
always@(*)
    case    (state)
        ...
        SEND_D_ADDR:
            if(bit_cnt <= 3'd6)
                i2c_sda_reg <=  DEVICE_ADDR[6 - bit_cnt];
            else
                i2c_sda_reg <=  1'b0;
        ACK_1:
            i2c_sda_reg <=  1'b1;
        SEND_B_ADDR_H:
            i2c_sda_reg <=  byte_addr[15 - bit_cnt];
        ACK_2:
            i2c_sda_reg <=  1'b1;
        SEND_B_ADDR_L:
            i2c_sda_reg <=  byte_addr[7 - bit_cnt];
        ACK_3:
            i2c_sda_reg <=  1'b1;
        WR_DATA:
            i2c_sda_reg <=  wr_data[7 - bit_cnt];
        ACK_4:
            i2c_sda_reg <=  1'b1;
        START_2:
            if(i2c_clk_cnt <= 2'd1)
        ...
        N_ACK:
            i2c_sda_reg <=  1'b1;
        STOP:
            if((bit_cnt == 3'd0) && (i2c_clk_cnt < 2'd3))
                i2c_sda_reg <=  1'b0;
            else
                i2c_sda_reg <=  1'b1;
        default:
...
always@(posedge i2c_clk or negedge sys_rst_n)
    if(sys_rst_n == 1'b0)
        rd_data <=  8'd0;
    else    if((state == RD_DATA) && (bit_cnt == 3'd7) && (i2c_clk_cnt == 2'd3))
        rd_data <=  rd_data_reg;
